...
cresize(int width, int height)
{
	...
	col = (win.w - 2 * (int)(usedfontsize*borderpx_mult)) / win.cw;
	row = (win.h - 2 * (int)(usedfontsize*borderpx_mult)) / win.ch;
	col = MAX(1, col);
	row = MAX(1, row);
...
xhints(void)
{
	...
	sizeh->height_inc = 1;
	sizeh->width_inc = 1;
	sizeh->base_height = 2 * (int)(usedfontsize*borderpx_mult);
	sizeh->base_width = 2 * (int)(usedfontsize*borderpx_mult);
	sizeh->min_height = win.ch + 2 * (int)(usedfontsize*borderpx_mult);
	sizeh->min_width = win.cw + 2 * (int)(usedfontsize*borderpx_mult);
	if (xw.isfixed) {
...
xinit(int cols, int rows)
{
	...
	win.w = 2 * win.hborderpx + 2 * (int)(usedfontsize*borderpx_mult) + cols * win.cw;
	win.h = 2 * win.vborderpx + 2 * (int)(usedfontsize*borderpx_mult) + rows * win.ch;
	if (xw.gm & XNegative)
...
xdrawglyphfontspecs(const XftGlyphFontSpec *specs, Glyph base, int len, int x, int y, int charlen)
{
	...
	if (do_glyph_clipping) {
		r.x = -glyph_clipping_margin;
		r.y = -glyph_clipping_margin;
		r.height = win.ch + glyph_clipping_margin * 2;
		r.width = width + glyph_clipping_margin * 2;
...
xximspot(int x, int y)
{
	...
	xw.ime.spot.x = (int)(usedfontsize*borderpx_mult) + x * win.cw;
	xw.ime.spot.y = (int)(usedfontsize*borderpx_mult) + (y + 1) * win.ch;
